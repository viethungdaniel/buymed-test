...
const AppYearButton = (
  props: PickersYearProps & { ownerState?: PickersYearProps }
) => {
  const { format, multiple, value } = useContext(AppDateCalendarContext);

  const { value: yearValue } = props?.ownerState ?? {};

  const selected = useMemo(() => {
    if (multiple) {
      return Array.isArray(value)
        ? value.some(
            (v) => `${moment(v, format).format("YYYY")}` === `${yearValue}`
          )
        : false;
    }
    return (
      !!value && `${moment(value, format).format("YYYY")}` === `${yearValue}`
    );
  }, [yearValue, format, multiple, value]);

  return <PickersYear {...props} selected={selected} />;
};

const AppMonthButton = (
  props: PickersMonthProps & { ownerState?: PickersMonthProps }
) => {
  const { format, multiple, value } = useContext(AppDateCalendarContext);

  const { value: monthValue } = props?.ownerState ?? {};
  ...
  const selected = useMemo(() => {
    if (multiple) {
      return Array.isArray(value)
        ? value.some(
            (v) =>
              `${moment(v, format).format("M")}` === `${monthValue ?? 0 + 1}`
          )
        : false;
    }
    return (
      !!value &&
      `${moment(value, format).format("M")}` === `${monthValue ?? 0 + 1}`
    );
  }, [monthValue, format, multiple, value]);

  return <PickersMonth {...props} selected={selected} />;
};
        ...
        >
          <DateCalendar
            ...
            slots={{
              ...
              yearButton: AppYearButton,
              monthButton: AppMonthButton,
