...
export const generateWhatsAppLink = (
  ...
  const urlEncodedMessage = !isEmpty(message)
    ? encodeURIComponent(message!)
    : "";
  return `${baseURL}${phoneNumber}${
    !!urlEncodedMessage ? `?text=${urlEncodedMessage}` : ""
  }`;
};

export const parseJSON = <D = any>(
  text: string,
  reviver?: (key: string, value: any, context: D) => D
): D | null => {
  try {
...
export const getAppUnitPrice = (price?: string | number | null) => {
